/**
 * Modified based on https://github.com/softnami/Autoencoder
 * replaced usage on mathjs.eval() to mathjs.evaluate()
 */
...
kullback_leibler_sum() {
  ...
  scope.p = this.p;
  scope.p = this.MathJS.evaluate('ones.*p',scope);
  scope.p_prime = this.p_prime;

  kl_matrix = this.MathJS.evaluate('((log(p./p_prime)).*p)+(log((1-p)./(1-p_prime)).*(1-p))', scope);
...
costFunction_Derivative(_X, _Y, W1, W2, iteration_count) {
  ...
  scope.kl_term = this.MathJS.evaluate('(((p./p_prime).*-1)+((1-p)./(1-p_prime))).*beta',scope);
  scope.kl_matrix = [];
  ...
  scope.arrA = this.MathJS.evaluate('arrA+kl_matrix',scope);
